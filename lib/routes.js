 ...
 * request:  curl -X POST <temporary_upload_link_url> --header "Content-Type:
 * application/octet-stream" --data-binary @local_file.txt  A successful
 * temporary upload link consumption request returns the content hash of the
 * uploaded data in JSON format. Example successful temporary upload link
...
/**
 * Asynchronous scene-change keyframe extraction for video files. Detects scene
 * changes in the source video and returns one representative keyframe per
 * detected scene, each tagged with its timestamp (seconds from the start of the
 * video) and scene-change score. Set `include_images = true` to also receive
 * each frame as a base64-encoded JPEG; when the field is omitted the response
 * carries keyframe metadata only. Supported video formats: .3gp, .3gpp, .3gpp2,
 * .asf, .avi, .dv, .flv, .m2t, .m2ts, .m4v, .mkv, .mov, .mp4, .mpeg, .mpg,
 * .mts, .mxf, .oggtheora, .ogv, .rm, .ts, .vob, .webm, .wmv. Unsupported
 * formats return an `unsupported_format_error`. Limits: the source file must be
 * at most 10 GB. To keep responses within service limits the number of
 * keyframes and the total image payload are bounded; requests that would exceed
 * these limits return a `limit_exceeded_error` -- raise
 * `scene_change_threshold` or set `include_images = false` to stay within
 * bounds.
 * Route attributes:
 *   scope: files.content.read
 * @function Dropbox#rivieraGetKeyframesAsync
 * @arg {RivieraGetKeyframesArgs} arg - The request parameters.
 * @arg {Object} [options] - The request options.
 * @returns {Promise.<DropboxResponse<AsyncLaunchResultBase>, DropboxResponseError.<void>>}
 */
routes.rivieraGetKeyframesAsync = function (arg, options) {
  return this.request('riviera/get_keyframes_async', arg, 'app, user', 'api', 'rpc', 'files.content.read', options);
};
 ...
 * Route attributes:
 *   scope: files.content.read
 * @function Dropbox#rivieraGetKeyframesAsyncCheck
 * @arg {AsyncPollArg} arg - The request parameters.
 * @arg {Object} [options] - The request options.
 * @returns {Promise.<DropboxResponse<RivieraGetKeyframesAsyncCheckResult>, DropboxResponseError.<AsyncPollError>>}
 */
routes.rivieraGetKeyframesAsyncCheck = function (arg, options) {
  return this.request('riviera/get_keyframes_async/check', arg, 'app, user', 'api', 'rpc', 'files.content.read', options);
};
 ...
 * MarkdownConversionApiV2Error.user_error. Size limit: the source file must be
 * at most 50 MB. Larger files fail with
 * MarkdownConversionApiV2Error.user_error. The markdown is not returned by this
 * route. Poll get_markdown_async/check with the returned async job ID until it
 * reports GetMarkdownAsyncCheckResult.complete or
 ...
 * format: .pdf. - MS Office formats: .docx, .pptx, .xlsx. Files in other
 * formats fail with MetadataExtractionApiV2Error.user_error. Size limits depend
 * on the kind of metadata being extracted: at most 200 MB for image (EXIF)
 * files, 100 GB for audio/video files, 500 MB for PDFs, and 288 MB for MS
 * Office files. Files over the limit for their kind fail with
 * MetadataExtractionApiV2Error.user_error. The metadata is not returned by this
 * route. Poll get_metadata_async/check with the returned async job ID until it
 * reports GetMetadataAsyncCheckResult.complete or
...
/**
 * Asynchronous OCR (optical character recognition) text extraction for images
 * and PDFs, including scanned / non-text PDFs. Supported formats: - Image
 * formats: .bmp, .gif, .heic, .jpeg, .jpg, .png, .tif, .tiff, .webp. - PDF
 * format: .pdf. Unsupported formats return an `unsupported_format_error`. For
 * the `url` variant only Dropbox shared links are supported; external URLs
 * return `unsupported_format_error`. Text-based PDFs already carry a text
 * layer, so OCR is not run against them and the result is empty; use
 * `get_text_async` to read the embedded text layer of such a PDF. The result
 * carries the extracted words as plain text, plus the same content as hOCR with
 * per-word coordinates.
 * Route attributes:
 *   scope: files.content.read
 * @function Dropbox#rivieraGetOcrAsync
 * @arg {RivieraGetOcrArgs} arg - The request parameters.
 * @arg {Object} [options] - The request options.
 * @returns {Promise.<DropboxResponse<AsyncLaunchResultBase>, DropboxResponseError.<void>>}
 */
routes.rivieraGetOcrAsync = function (arg, options) {
  return this.request('riviera/get_ocr_async', arg, 'app, user', 'api', 'rpc', 'files.content.read', options);
};
 ...
 * Route attributes:
 *   scope: files.content.read
 * @function Dropbox#rivieraGetOcrAsyncCheck
 * @arg {AsyncPollArg} arg - The request parameters.
 * @arg {Object} [options] - The request options.
 * @returns {Promise.<DropboxResponse<RivieraGetOcrAsyncCheckResult>, DropboxResponseError.<AsyncPollError>>}
 */
routes.rivieraGetOcrAsyncCheck = function (arg, options) {
  return this.request('riviera/get_ocr_async/check', arg, 'app, user', 'api', 'rpc', 'files.content.read', options);
};
 ...
 * .rm, .ts, .vob, .webm, .wmv. Files in other formats fail with
 * ContentApiV2Error.user_error. Size limits: the source file must be at most 10
 * GB and its audio track at most 1 hour in duration. Files exceeding either
 * limit fail with ContentApiV2Error.user_error. The transcript is not returned
 * by this route. Poll get_transcript_async/check with the returned async job ID
